# Requirements Archive: v1.3 Docker + Load Balancing

**Archived:** 2026-03-22
**Status:** SHIPPED

All 13 requirements delivered. See traceability table below.
...
- [x] **CONT-01**: App reads `DATABASE_URL` env var for Postgres connection (fallback to `postgres://admin:pass@localhost:15432/postgres` for local dev)
- [x] **CONT-02**: App exposes `GET /health` endpoint returning HTTP 200
- [x] **CONT-03**: App handles `SIGTERM` for graceful shutdown alongside existing `SIGINT` handler

### Docker Image

- [x] **DOCK-01**: Multi-stage Dockerfile uses `oven/bun:1.3.11` for both builder and runtime stages
- [x] **DOCK-02**: `.dockerignore` excludes `node_modules`, `.git`, `.env` from build context
- [x] **DOCK-03**: Runtime image copies only production deps + `src/` (no devDependencies in final layer)
